  ...
  if (imageRef.value) {
    ...
    if (!props.node.attrs.width) {
      ...
      if (!props.node.attrs.height && naturalWidth.value && !isInline.value) {
        if (hasCrop.value) {
          const natCropW = naturalWidth.value * visibleRatioW.value
          parsedWidth = Math.min(natCropW, editorMaxWidth.value)
        } else {
      ...
      if (parsedHeight) {
        const expectedRatio = hasCrop.value
          ? (naturalHeight.value * visibleRatioH.value) / (naturalWidth.value * visibleRatioW.value)
        ...
        if (expectedRatio && Math.abs(actualRatio - expectedRatio) > 0.02) {
          logger.warn('[image] height mismatch, drop height', {
            ...
            naturalWidth: naturalWidth.value, naturalHeight: naturalHeight.value
          })
